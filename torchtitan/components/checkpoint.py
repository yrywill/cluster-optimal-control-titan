...
class CheckpointManager(Configurable):
    ...
    def __init__(
        ...
        *,
        ...
        self.hf_assets_path: str | None = None  # set by trainer after init
    ...
    def dcp_save(
        ...
        if to_hf:
            ...
            hf_dir = os.path.join(checkpoint_id, "hf")
            if fqn_to_index_mapping:
                storage_writer = HuggingFaceStorageWriter(
                    path=os.path.join(hf_dir, "sharded"),
        ...
        if to_hf and fqn_to_index_mapping:
            consolidate_safetensors_files_on_every_rank(
                input_dir=os.path.join(hf_dir, "sharded"),
                output_dir=hf_dir,
                fqn_to_index_mapping=fqn_to_index_mapping,
                num_threads=5,
            )

        # Copy HF config/tokenizer files so the checkpoint is a complete
        # HF model directory ready for inference.
        if to_hf and self.hf_assets_path:
            ...
            if rank == 0:
                ...
                for fname in _hf_files:
                    src = os.path.join(self.hf_assets_path, fname)
                    if os.path.exists(src):
                        shutil.copy2(src, os.path.join(hf_dir, fname))
                logger.info(
                    "Copied HF config/tokenizer files to %s", hf_dir,
                )
    ...
    def _create_checkpoint_id(self, step: int, folder: str = "") -> str:
        folder = folder if folder else self.folder
        return os.path.join(folder, f"global_step{step}")
